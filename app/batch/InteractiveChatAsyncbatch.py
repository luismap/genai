...
def process_batch(batch: List[Tuple[Awaitable[ChatBotReadModel], ChatBotPayloadModel]]) -> None:
...
async def batch_processing_loop(loop: asyncio.AbstractEventLoop):
	...
	with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
		while True:
			...
			current_batch: List[Tuple[Awaitable[ChatBotReadModel],ChatBotPayloadModel]] = [await queue.get()]
