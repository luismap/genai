...
from features.chatbot.data.models.AudioDataModel import (
    AudioDataPayloadModel,
    AudioDataReadModel,
...
def chat(input_text):
    ...
    add_row(
        f"`{datetime.datetime.now()}`- exec time: `{inference_time}s` - llm model: `{model_use}` - quatization bitmode: `{qbm}`",
        "rows",
    )
    add_row("=" * 50, "rows")
...
def transcribe(file: Path, language: str):
    full_name = str(file)
    payload = AudioDataPayloadModel(
        source_audio=full_name, language=language, task="transcribe"
    )
    ...
    add_row(
        f"`{datetime.datetime.now()}`- exec time: `{inference_time}s` - audio model: `{data.model}` - file : `{file}`",
        "rows_audio",
    )
...
def translate(file: Path, language: str):
    full_name = str(file)
    payload = AudioDataPayloadModel(
        source_audio=full_name, language=language, task="translate"
    )
...
def vdb_simsearch(text: str):
    ...
    try:
        ...
        st.session_state.vdb_simsearch_content = (
            "No content Found, please expand the search"
        )
...
get_context_button = ctx_button.button(
    "🔤 get approximate context length", key="gcb_qa"
)

ctx_content.write(
    f"current context length: :green[{st.session_state.qa_context_length}] tokens"
)
...
# using button call to trigger refresh
get_context_button_rag = ctx_button_rag.button(
    "🔤 get approximate context length", key="gcb_rag"
)

ctx_content_rag.write(
    f"current context length: :green[{st.session_state.rag_context_length}] tokens"
)
...
uploader = col_upload.file_uploader(
    "choose text file(s) to upload", accept_multiple_files=True
)
...
with audio_tab_right.form("audio-uploader-form", clear_on_submit=True):
    audio_uploader = st.file_uploader(
        "choose audio file(s) to upload (wav, flac or mp3)", accept_multiple_files=True
    )
    ...
    if submitted and audio_uploader is not None:
        for uploaded_file in audio_uploader:
            ...
            if trgt_path.suffix not in (".wav", ".flac", ".mp3"):
                add_row(
                    f":red[ERROR] - :file {filename} is no a supported format",
                    "rows_audio_log",
                )
                continue
...
translate_language = translate_lang_col.text_input(
    "🗣️ choose translate language", value="english"
)

checkbox_labels = {p for p in st.session_state.audio_files}
task_labels = {"transcribe", ""}
option = audio_tab_main.selectbox("Choose and audio file to be played", checkbox_labels)
transcribe_col_bc, translate_bc, cancel_bc, _ = audio_tab_main.columns(
    [0.2, 0.2, 0.2, 0.6]
)
...
if option != None:
    ...
    text = "Transcribe"
    transcribe_col_bc.button(
        f"{text: ^12}", type="primary", on_click=transcribe, args=(path, language)
    )
