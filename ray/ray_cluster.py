...
class RayCluster:
        ...
        for workload_details in [self.ray_head_details, self.ray_worker_details]:
            for status in ["workers", "failures"]:
                if workload_details is not None and status in workload_details:
                    stop_responses = cdsw.stop_workers(
                        *[workload["id"] for workload in workload_details[status]]
                    )
                    stop_response_statuses = [
                        response.status_code for response in stop_responses
                    ]
                    if any([status >= 300 for status in stop_response_statuses]):
                        print(
                            "Could not stop all Ray workloads. Trying to force stop all CML workers created in this session."
                        )
                        cdsw.stop_workers()
    ...
    def _start_ray_workload(self, args, startup_timeout_seconds):
        ...
        if len(workloads_with_no_ids) > 0:
            print(
                "Could not create all requested workloads. Error messages received:",
                no_id_messages,
            )
            # Clean up all workloads
            ids = [wl["id"] for wl in workloads_with_ids] + [
                wl.get("engineId", "") for wl in workloads_with_no_ids
            ]
    ...
    def _start_ray_head(self, startup_timeout_seconds):
        ...
        head_start_cmd = f"!export PATH=$PATH:/home/cdsw/.local/bin; echo $PATH; ray start --head --block --disable-usage-stats --num-cpus={self.head_cpu} --num-gpus={self.head_nvidia_gpu} --include-dashboard=true --dashboard-port={self.dashboard_port}"
    ...
    def _add_ray_workers(self, head_addr, startup_timeout_seconds):
        # We need to start the ray process with --block else the command completes and the CML Worker terminates
        worker_start_cmd = f"!export PATH=$PATH:/home/cdsw/.local/bin; echo $PATH;ray start --block --num-cpus={self.worker_cpu} --num-gpus={self.worker_nvidia_gpu} --address={head_addr}"
        ...
        self.ray_worker_details = self._start_ray_workload(
    ...
    def init(self, startup_timeout_seconds=90):
        ...
        if len(self.ray_head_details.get("workers", [])) < 1:
            ...
            self._add_ray_workers(
                ray_head_addr, startup_timeout_seconds=startup_timeout_seconds
            )
            if (
                self.ray_worker_details is None
                or len(self.ray_worker_details.get("workers", [])) < self.num_workers
            ):
